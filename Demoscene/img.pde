void img() {
  
    jolka = loadImage("happynewyear/jolka.png");
    zvezda = loadImage("happynewyear/zvezda.png");
    sugrob = loadImage("happynewyear/sugrob.png");
    sprite = loadImage("data/sprite.png");
    
    hny[0]  = loadImage("happynewyear/00.gif");
    hny[1]  = loadImage("happynewyear/10.gif");
    hny[2]  = loadImage("happynewyear/20.gif");
    hny[3]  = loadImage("happynewyear/30.gif");
    hny[4]  = loadImage("happynewyear/40.gif");
    hny[5]  = loadImage("happynewyear/50.gif");
    hny[6]  = loadImage("happynewyear/60.gif");
    hny[7]  = loadImage("happynewyear/70.gif");
    hny[8]  = loadImage("happynewyear/80.gif");
    hny[9]  = loadImage("happynewyear/90.gif");
    hny[10]  = loadImage("happynewyear/100.gif");
    hny[11]  = loadImage("happynewyear/110.gif");
    hny[12]  = loadImage("happynewyear/120.gif");
    hny[13]  = loadImage("happynewyear/130.gif");
    hny[14]  = loadImage("happynewyear/140.gif");
    hny[15]  = loadImage("happynewyear/150.gif");
      
    images[0]  = loadImage("deadmoroz/0.gif");
    images[1]  = loadImage("deadmoroz/1.gif"); 
    images[2]  = loadImage("deadmoroz/2.gif");
    images[3]  = loadImage("deadmoroz/3.gif"); 
    images[4]  = loadImage("deadmoroz/4.gif");
    images[5]  = loadImage("deadmoroz/5.gif"); 
      
    images2[0]  = loadImage("girlyanda/01.gif");
    images2[1]  = loadImage("girlyanda/11.gif");
    images2[2]  = loadImage("girlyanda/21.gif");
    images2[3]  = loadImage("girlyanda/31.gif");
    images2[4]  = loadImage("girlyanda/41.gif");
    images2[5]  = loadImage("girlyanda/51.gif"); 
  
}